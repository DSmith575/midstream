...
export const ServicePlanSummary = ({
  ...
  const allocatedMinutes = serviceList.reduce(
    (
      minutes: { total: number; fixed: number; flexible: number },
      currentValue: any,
    ) => {
  ...
  return (
    <section className="flex flex-col items-center">
      <div className="grid grid-cols-3 gap-6 w-full max-w-s justify-items-center">
        <HourStat
          label="Fixed Hours"
          value={(allocatedMinutes.fixed / 60).toFixed(1)}
        />
        <HourStat
          label="Flexible Hours"
          value={(allocatedMinutes.flexible / 60).toFixed(1)}
        />
        <HourStat
          label="Total Hours"
          value={(allocatedMinutes.total / 60).toFixed(1)}
        />
      </div>
    </section>
  )
