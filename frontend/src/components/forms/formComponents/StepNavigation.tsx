...
const getStepClasses = (stepIndex: number, currentStep: number) => {
  ...
  const borderColor = isCompleted || isCurrent ? 'border-sky-600' : 'border-gray-200'
...
const StepItem = ({ step, index, currentStep }: { step: { id: string; name: string }; index: number; currentStep: number }) => {
  const { baseClasses, textColor } = getStepClasses(index, currentStep)
  const isCurrent = currentStep === index

  return (
    <li key={step.name} className="md:flex-1">
      <div className={baseClasses} {...(isCurrent && { 'aria-current': 'step' })}>
    ...
    <nav aria-label="Progress" className="mb-8 mt-4 mx-4">
      <ol role="list" className="space-y-4 md:flex md:space-x-8 md:space-y-0">
        {steps.map((step, index) => (
          <StepItem key={step.name} step={step} index={index} currentStep={currentStep} />
        ))}
      </ol>
    </nav>
  )
