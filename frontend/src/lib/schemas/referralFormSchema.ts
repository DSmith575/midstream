...
export const referralFormSchema = z.object({
  ...
  .int("Postcode must be a number"),
	...
	firstLanguage: z.string().nonempty({message: "A language must be selected"}),
	interpreter: z.string().nonempty({message: "Please select an option."}),
	culturalSupport: z.string().nonempty({message: "Please select an option."}),
	communicationNeeds: z.string().nonempty(),
});