...
export const referralFormSchema = z
	.object({
		...
		communicationNeeds: z
			.string()
			.nonempty({ message: "Please select an option." }),
		communicationNeedsDetails: z.string().min(0).max(150, {message: "150 character limit"}).optional(),
		doctorName: z.string().nonempty({ message: "Doctor/GP name is required" }),
		doctorPhone: z.string().nonempty({ message: "Doctor/GP phone is required" }),
		doctorAddress: z.string().nonempty({ message: "Medical Centre address is required" }),
		doctorSuburb: z.string().nonempty({message: "Medical Centre suburb is required"}),
		doctorCity: z.string().nonempty({message: "Medical Centre city is required"}),
		nationalHealthIndex: z.string().nonempty({message: "NHI Number required"}).toUpperCase(),
	})
	.refine(
		(data) =>
